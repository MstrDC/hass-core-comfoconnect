...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # original name is stored in original_name. Only clear the integration-owned
    # metadata so entities can pick up the new descriptions without losing user
    # customizations.
    ...
    for entity in ent_reg.entities.values():
        if entity.config_entry_id == entry.entry_id and entity.platform == DOMAIN:
            if entity.original_name is not None:
                _LOGGER.debug(
                    "Clearing stale original name %s -> %s",
                    entity.entity_id,
                    entity.original_name,
                )
                ent_reg.async_update_entity(entity.entity_id, original_name=None)
